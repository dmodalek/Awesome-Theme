...
/*
 * Generated with Grunt on 26.11.2014 at 17:40:45
 */
...
function setFullHeight($els, resize) {
    var resizeListener = "undefined" != typeof resize ? resize : !0, $window = $(window), windowHeight = $window.height() - $(".header").height();
    $.each($els, function(index, el) {
        windowHeight > $(el).height() && $(el).css({
...
window.Modernizr = function(window, document, undefined) {
    ...
    var featureName, hasOwnProp, version = "2.8.3", Modernizr = {}, /*>>cssclasses*/
 ...
 */
        ...
        registerModules: function($ctx) {
             ...
             */
                if (classes = classes.split(" "), classes.length > 1) {
                    for (var modName, dataConnectors, skins = [], connectors = [], i = 0, len = classes.length; len > i; i++) {
                 ...
                 */
                    dataConnectors = $this.attr("data-connectors")) {
                        ...
                        for (var i = 0, len = connectors.length; len > i; i++) {
     ...
     */
        unregisterModules: function(modules) {
            ...
            for (var i = 0, len = modules.length; len > i; i++) {
     ...
     */
        start: function(modules) {
            ...
            // Start the modules
            for (var i = 0, len = modules.length; len > i; i++) modules[i].start();
        },
        /**
     ...
     */
        stop: function(modules) {
            modules = modules || this.modules;
            // Stop the modules
            for (var i = 0, len = modules.length; len > i; i++) modules[i].stop();
        },
        /**
     * Registers a module.
     *
     ...
     */
        registerModule: function($node, modName, skins, connectors) {
            ...
            modName && Tc.Module[modName]) {
                ...
                // Decorate it
                for (var i = 0, len = skins.length; len > i; i++) {
                    var skinName = skins[i];
                    Tc.Module[modName][skinName] && (modules[id] = modules[id].getDecoratedModule(modName, skinName));
                }
                // Register connections
                for (var i = 0, len = connectors.length; len > i; i++) this.registerConnection(connectors[i], modules[id]);
                return modules[id];
            }
            return null;
        },
 ...
 */
     ...
     */
        fire: function(state, data, channels, defaultAction) {
            ...
            for (var i = 0, len = channels.length; len > i; i++) {
 ...
 */
     ...
     */
        notify: function(origin, state, data) {
            /*
...
function($) {
    $(document).ready(function() {
        var $html = $("html");
        window.application = new Tc.Application($html), application.registerModules($html), 
        application.start(), console.log("App started");
    });
}(Tc.$), /**
 * Dev Tools
 */
function() {
    var addDebugBadges = function() {
        ...
        $badges.on("click", function(e) {
            var $badge = $(e.target), type = $badge.attr("href").replace("#", "");
            e.preventDefault(), toggleState($badge), // Toggle Hash
            toggleHash(type), // Update Mod Outlines
            updateModOutline();
        });
    }, activateBadges = function() {
        var $badge, types, $ctx = $("body"), currHash = window.location.hash;
        0 !== currHash.length && (types = currHash.replace("#", "").split("+"), $.each(types, function(index, type) {
            $badge = $(".badge-" + type, $ctx), // proceed if there a badge with this class
            $badge.length && toggleState($badge);
        }));
    }, onResize = function() {
        $(window).resize(function() {
            updateModOutline();
        });
    }, toggleState = function($badge) {
        var $html = $("html"), $ctx = $("body"), type = $badge.attr("href").replace("#", "");
        // Toggle active class on badge
        $badge.toggleClass("active"), // Toggle debug-type class on html / body
        "va" == type && $ctx.toggleClass("debug-" + type), "mod" == type && ($html.toggleClass("debug-" + type), 
        addModOutline()), "grid" == type && $html.toggleClass("debug-" + type);
    }, toggleHash = function(type) {
        var newHash, currHash = window.location.hash;
        // Add Hash
        if (-1 === currHash.indexOf(type)) newHash = 0 === currHash.length ? type : currHash + "+" + type; else {
            var pos = currHash.indexOf(type);
            "+" == currHash.charAt(pos - 1) && (newHash = currHash.replace("+" + type, "")), 
            "#" == currHash.charAt(pos - 1) && currHash.length !== type.length && (newHash = currHash.replace(type + "+", "")), 
            "#" == currHash.charAt(pos - 1) && currHash.length == type.length + 1 && (newHash = "");
        }
        window.location.hash = newHash;
    }, addModOutline = function() {
        var $ctx = $("html");
        $ctx.hasClass("debug-mod") ? $(".mod:not(.mod-layout):visible").each(function() {
            var $this = $(this), position = $this.offset(), dimension = {
                height: $this.outerHeight(),
                width: $this.outerWidth()
            }, positioning = $this.css("position"), classes = $this.attr("class").split(" "), name = "";
            if (classes.length > 1) for (var i = 0, len = classes.length; len > i; i++) {
                var part = $.trim(classes[i]);
                0 === part.indexOf("mod") && part.length > 3 && (name = part.substr(4));
            }
            ("static" == positioning || "relative" == positioning) && (positioning = "absolute");
    ...
    }, updateModOutline = function() {
        $(".terrific-module").remove(), addModOutline();
    };
    $("body").hasClass("layout-dev") && (// Insert debug badges into page
    addDebugBadges(), // Click on a badge
    onBadgeClick(), // Activate a badge on page load
    activateBadges(), // Resize
    onResize());
}(Tc.$), function() {
    "use strict";
    ...
    Tc.Module.MainNav = Tc.Module.extend({
        init: function($ctx, sandbox, modId) {
            ...
            // Do stuff here
            //...
            $(".mobile-toggle", this.$ctx);
        },
        on: function(callback) {
            // Do stuff here
            //...
            var $mobileToggle = $(".mobile-toggle", this.$ctx);
            $mobileToggle.on("click", this.onToggleMobileNav), // callback
            callback();
        },
        onToggleMobileNav: function(ev) {
            var $mobileToggle = $(ev.delegateTarget), $ctx = this.$ctx, $navList = $(".nav-list-l1", $ctx), $navHeight = $navList.outerHeight();
            $mobileToggle.toggleClass("active"), "none" !== $navList.css("display") ? // Hide
            $navList.animate({
                height: 0
